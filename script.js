(() => {
  const dailyTaskData = [{id:"daily_guild_checkin",label:"Guild Check-In & Cargo (Guild Administrator & Cargo Administrator in Guild Center)",color:"orange"},{id:"daily_unstable_space_dungeon",label:"Clear Unstable Space Dungeon (2x Daily)",color:"purple"},{id:"daily_season_activity_goals",label:"Season Pass Activity (Earn 500 Activity Merits in the Season Pass)",color:"yellow"},{id:"daily_commissions",label:"Bureau Commissions (3x Daily | You can skip this daily for up to 2 days | Commissions Cap: 9)",color:"green"},{id:"daily_mystery_store",label:"Mystery Store (Buy Starforge Crystals & Advanced Books & Moss/Burl Shards if available)",color:"grey"},{id:"daily_world_boss_keys",label:"World Boss Keys (2x Daily | You can skip this daily for up to 2 days | Keys Cap: 6)",color:"red"},{id:"daily_elite_boss_keys",label:"Elite Boss Keys (2x Daily | You can skip this daily for up to 2 days | Keys Cap: 6)",color:"red"},{id:"daily_focus",label:"Life Skill Focus (Spend 400 Focus Daily | This daily can be skipped for up to 4 days | Focus Cap: 2000)",color:"yellow"},{id:"daily_homestead_commissions",label:"Homestead Commissions (You can skip this daily for up to 2 days)",color:"green"}];

  const weeklyTaskData = [{id:"weekly_pioneer_rewards",label:"Pioneer Awards (Pioneer NPC in town)",color:"yellow"},{id:"weekly_reclaim_hub",label:"Reclaim Hub (If missed a daily/weekly)",color:"grey"},{id:"weekly_guild_activity_rewards",label:"Guild Activity Rewards (7000/7000 Points)",color:"orange"},{id:"weekly_guild_hunt_extended",label:"Guild Hunt (Available only on Friday, Saturday, Sunday)",color:"orange"},{id:"weekly_guild_dance",label:"Guild Dance (Available only on Friday)",color:"orange"},{id:"weekly_world_boss_crusade_points",label:"World Boss Crusade (Available Daily | Earn 1200 Points)",color:"red"},{id:"weekly_clear_dungeons_normal",label:"Clear Dungeons (Normal/Difficult - 20 runs for weekly cap of Reforge Stones)",color:"purple"},{id:"weekly_clear_dungeons_master_1_5",label:"Clear Dungeons (Master 1-5 - 20 runs for weekly cap of Reforge Stones)",color:"purple"},{id:"weekly_clear_dungeons_master_6_20",label:"Clear Dungeons (Master 6-20 - 20 runs for weekly cap of Reforge Stones)",color:"purple"},{id:"weekly_fight_bane_lord",label:"Fight the Bane Lord (Random Encounter when clearing dungeons | Max 5 times a week for Legendary Select Boxes)",color:"red"},{id:"weekly_gear_exchange_store",label:"Gear Exchange Store (Buy Luno Pouches & Allow Shards from all the gear exchange stores)",color:"grey"},{id:"weekly_honor_store",label:"Honor Store (10000 Honor Points weekly)",color:"grey"},{id:"weekly_friendship_store",label:"Friendship Store (2000 Friendship Points weekly)",color:"grey"},{id:"weekly_reputation_store",label:'Reputation Store (Buy Will Wish Coin & "Revive" Candy & Healing Aromatic Lv.1)',color:"grey"},{id:"weekly_guild_store",label:"Guild Store (Buy Focus Potions & Guild Supply Chests & Burl Shards)",color:"grey"},{id:"weekly_event_store",label:"Event Store (if its available)",color:"grey"},{id:"weekly_life_skill_quests",label:"Life Skill Quests (12 Exchange Quests)",color:"green"},{id:"weekly_stimen_vaults",label:"Stimen Vaults (Resets every 2 weeks)",color:"silver"},{id:"weekly_ice_dragon_normal",label:"Ice Dragon Raid - Normal (12710+ Ability Score)",color:"blue"},{id:"weekly_ice_dragon_hard",label:"Ice Dragon Raid - Hard (16140+ Ability Score)",color:"blue"},{id:"weekly_ice_dragon_nightmare",label:"Ice Dragon Raid - Nightmare (22300+ Ability Score)",color:"blue"},{id:"weekly_dark_dragon_normal",label:"Dark Dragon Raid - Normal (15210+ Ability Score)",color:"dark_purple"},{id:"weekly_dark_dragon_hard",label:"Dark Dragon Raid - Hard (19040+ Ability Score)",color:"dark_purple"},{id:"weekly_dark_dragon_nightmare",label:"Dark Dragon Raid - Nightmare (24180+ Ability Score)",color:"dark_purple"},{id:"weekly_light_dragon_normal",label:"Light Dragon Raid - Normal (UNKNOWN Ability Score)",color:"yellow"},{id:"weekly_light_dragon_hard",label:"Light Dragon Raid - Hard (UNKNOWN Ability Score)",color:"yellow"},{id:"weekly_light_dragon_nightmare",label:"Light Dragon Raid - Nightmare (UNKNOWN Ability Score)",color:"yellow"}];

  const $ = id => document.getElementById(id);

  let profiles = {current:"default",list:["default"],data:{default:{weekly_tasks:{}}}};
  let isStorageAllowed = localStorage.getItem('gdpr_optout') !== 'true';

  const saveProfiles = () => isStorageAllowed && localStorage.setItem('checklist_profiles', JSON.stringify(profiles));
  ...
  const renderProfilesList = () => {
    ...
    profiles.list.sort().forEach(name => {
      ...
      switchBtn.onclick = () => {profiles.current = name; saveProfiles(); reloadCurrentProfileData(); profilesModal.style.display='none';};
  ...
  profilesBtn.onclick = () => {currentProfileNameEl.textContent=profiles.current; renderProfilesList(); profilesModal.style.display='flex';};
  ...
  const toggleTask = (el, section) => {
    ...
    if(section==='weekly'){
      ...
      saveProfiles();
    }else updateDailyStorage(id, completed);
  ...
  const updateCounter = section => {
    ...
    if(done===total&&total){
      ...
      if(!progress.dataset.confettiDone){
        ...
        confetti({particleCount:500,spread:360,origin:{x:.5,y:-.5}});
      }
    }else{
  ...
  const exportProgress = () => {
    ...
    const pd = getProfileData();
    const data = {profile:profiles.current,daily_tasks:pd.daily_tasks||null,weekly_reset_date:pd.weekly_reset_date||null,weekly_tasks:pd.weekly_tasks||{}};
    const blob = new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `bp_${profiles.current}_${new Date().toISOString().slice(0,10)}.json`;
  ...
  const updateTitle = () => {
    const launch = new Date('2025-10-09T10:00:00-02:00');
    const now = new Date();
  ...
  const getDailyReset = now => {
    const r = new Date(now); r.setHours(5,0,0,0);
  ...
  const getGuildHuntPeriod = now => {
    ...
    for(const day of days){
      ...
      end.setDate(end.getDate()+1); end.setHours(4,0,0,0);
  ...
  class EventTimer{
    constructor(id,getPeriod,label){
      this.el=$(id); this.getPeriod=getPeriod; this.label=label;
  ...
  const startTimerUpdates = () => {const updateAll=()=>timers.forEach(t=>t.update()); updateAll(); setInterval(updateAll,1000);};

  const checkGDPR = async () => {
    try{
      const res=await fetch('https://ipapi.co/json/');
      const data=await res.json();
      if(data.continent_code==='EU' && !localStorage.getItem('gdpr_consent')) gdprModal.style.display='flex';
    }catch{}
  };

  acceptBtn.onclick = () => {localStorage.setItem('gdpr_consent','true'); isStorageAllowed=true; gdprModal.style.display='none';};
  rejectBtn.onclick = () => {
    localStorage.setItem('gdpr_optout','true'); localStorage.clear(); isStorageAllowed=false;
  ...
  const init = async () => {
    ...
    toggleDailyBtn.onclick = () => {hideCompletedState.daily=!hideCompletedState.daily; toggleDailyBtn.textContent=hideCompletedState.daily?'Show Completed':'Hide Completed'; applyCompletedFilter('daily');};
    toggleWeeklyBtn.onclick = () => {hideCompletedState.weekly=!hideCompletedState.weekly; toggleWeeklyBtn.textContent=hideCompletedState.weekly?'Show Completed':'Hide Completed'; applyCompletedFilter('weekly');};
    ...
    dailyFilterInput.oninput = weeklyFilterInput.oninput = () => {applyCompletedFilter('daily'); applyCompletedFilter('weekly');};
  };
