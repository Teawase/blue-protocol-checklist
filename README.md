...
A simple, responsive web-based checklist for tracking daily and weekly tasks in Blue Protocol: Star Resonance. Built with HTML, CSS, and vanilla JavaScript. No external dependencies beyond confetti.js for celebrations. Progress is saved locally in your browser.

# Features
## Core Checklist Functionality
- **Daily Tasks:** Track your daily activities (e.g., Guild Check-In, Unstable Space Dungeon clears) with color-coded categories (e.g., red for boss keys, green for commissions).
- **Weekly Tasks:** Track your weekly activities (e.g., Pioneer Awards, Dragon Raids) with similar color-coding and organization.
...
## Search & Navigation
- **Live Search:** Debounced text input filters tasks by label in both daily and weekly sections; layout switches to vertical list during filtering for better readability.
...
- **Daily Streak Counter:** Automatically calculates and displays consecutive days of full daily completion (up to 30 days back, capped at 60-day history). Shown next to the Daily Tasks header.
...
- **Smart Updates:** Refreshes every second when visible, every 5 seconds when tabbed away; indicates active periods with styling changes.
- **Clickable Refresh:** Click any timer name to force-update all timers.
...
- **Color-Coded UI:** Tasks use distinct backgrounds (e.g., purple for dungeons, blue for raids) for quick visual grouping.
...
## Privacy & Edge Cases
- **GDPR Compliance:** Auto-detects EU visitors via IP API; shows a modal for consent. Opt-out clears all data, closes the tab, and displays a blocking message (with fallback close button).
...
## Resources & Extras
- **Footer Links:** Quick access to community resources (e.g., Blue Protocol Database, Maxroll guides, interactive map).
- **No Internet Required:** Fully offline after initial load (except GDPR check); timers use browser Date API.

## Tech Stack
- **HTML5:** Semantic structure with modals and live regions.
- **CSS3:** Flexbox, gradients, animations, dark theme (custom properties ready for light mode).
...
- **Tasks:** Add new dailies/weeklies, fix typos.
- **Timers:** Add new events, fix timezone logic.
- **UI/UX:** Improve mobile layout, enhance accessibility.
- **Features:** Export/import progress, streak achievements, notifications, sound effects.
- **Performance:** Optimize localStorage.
